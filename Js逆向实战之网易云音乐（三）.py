...
class WangyiComments():
    ...
    def catch(self,page):
        ...
        self.write_csv('hotComments')
        self.catch_originComments(page)
        # self.print_data()
        self.write_csv('originComments')
    ...
    def print_data(self):
        for data in self.origin_list:
            print(data)
        print('-' * 100)
        for data in self.hot_list:
            print(data)

    def write_csv(self,name):
        if not os.path.exists(self.file_loc):
            os.mkdir(self.file_loc)
        open(f'{self.file_loc}/{name}.csv','a',encoding = 'utf-8').write('names\tcontents\n')
        if name[0] == 'h':
            with open(f'{self.file_loc}/{name}.csv','a',encoding = 'utf-8') as fp:
                for data in self.hot_list:
                    fp.write('%s\t%s\n'%(data[0],data[1]))
        else:
            with open(f'{self.file_loc}/{name}.csv','a',encoding = 'utf-8') as fp:
